...
public class Game {
    ...
    private int playerEnemiesDestroyed = 0;
    ...
    public Game tick(String itemUsedId) throws InvalidActionException {
        ...
        if (!(item instanceof Bomb) && !(item instanceof Potion) && !(item instanceof LogicalBomb))
        ...
        registerOnce(() -> {
            ...
            if (item instanceof LogicalBomb)
                player.use((LogicalBomb) item, map);
    ...
    public <T extends Entity> List<T> getAllEntityType(Class<T> type) {
